...
*/
 ...
 int main(int argc, char *argv[])
 {
  ...
  unsigned char *key[WORD_SIZE] ;   /* current word read in from the file */
	...
	while ( !Found && index != 0 )
	  {
		...
		if ( !Found && !Move ) fscanf(searchfile, "%d", &index) ;

		/* if not at a leaf, go to the next record */
		if ( index != 0 && fseek(searchfile, (index-1)*RECORD_SIZE, SEEK_SET) != 0 )
			{
			 fprintf(stderr, "File I/O error: unable to seek in file '%s'. \n", argv[argc-1]) ;
			 exit(3) ;    /* error if couldn't reposition properly */
			}
	  } /* endwhile-outer */
