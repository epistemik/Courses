...
*/
 ...
 int main(int argc, char *argv[])
 {
  int wordcount,        	/* index of current word to search for */
		...
		Position,				/* +ve or -ve value from strcmp for key vs. word */
  ...
  if ( fseek(searchfile, 0, SEEK_END) != 0 )
	 {
	  fprintf(stderr, "File I/O error: unable to find EOF for file '%s'. \n", argv[argc-1]) ;
	  exit(3) ;
	 }
  ...
  {
	midpoint = number_of_records / 2 ;  /* set initial midpoint */
	startpoint = 0 ;                    /* set initial startpoint */
	endpoint = number_of_records + 1 ;  /* set initial endpoint */
	Found = FALSE ;     	/* initialize Found to FALSE for each word */
	recordcount = 0 ;    /* initial record count is zero */

	while ( !Found && (startpoint < midpoint+1) )
	/* as long as current key not found and records remain in the search interval */
	  {
		...
		midpoint = ((startpoint + endpoint)/2) - 1 ;
