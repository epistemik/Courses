...
void OUTPUT(int, int, int *, double *, double [][11]);
...
main()
{
	...
	double AMAX, XM, SUM ;
	int NROW[10] ;
	int N, M, ICHG, I, NN, IMAX, J, JJ, IP, JP, NCOPY, I1, J1, N1, K, N2, KK, OK;
...
void INPUT(int *OK, double A[][11], int *N)
{
   ...
   FILE *INP; 
	...
	if ((AA == 'Y') || (AA == 'y')) {
		...
		printf("for example:   D:\\DIRECTORY\\DATA.DTA  \n");
		scanf("%s", NAME);
		INP = fopen(NAME, "r");
		*OK = false;
		while (!(*OK)) {
...
void OUTPUT(int N, int ICHG, int *NROW, double *X, double A[][11])
	{
	int I, J, FLAG;
	char NAME[30];
	FILE *OUP;
	...
	if (FLAG == 2)
		{
		printf("Input the file name in the form - drive:\\dir\\name.ext \n");
		printf("for example:  D:\\DIRECTORY\\DATA.DTA  \n");
		scanf("%s", NAME);
		OUP = fopen(NAME, "w");
		}
