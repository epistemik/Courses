...
main()
{
   ...
   if (OK) { 
      ...
      printf("\n Initial Data: \n") ; 
      printf(" ------------- \n") ; 
      printf(" Limits of integration = [%11.8f, %11.8f] \n", A, B);
...
void INPUT(int *OK, double *A, double *B, int *N)
{
   ...
   printf(" Has the function F been defined in this program? \n");
   printf(" Please enter Y or N: ");
   scanf("%c",&AA);
   if ((AA == 'Y') || (AA == 'y')) {
      *OK = false;
      while (!(*OK)) {
	 printf("\n Input lower limit of integration and ");
      ...
      *OK = false;
      while (!(*OK)) {
	 printf("\n Enter a positive integer for number of rows: ");
	 scanf("%d", N);
	 if (*N > 0) *OK = true;
