...
%
% member(?X,?Y)
%   Succeeds if X is a member of the list Y.
%
...
dlisting(Predicate) :-
        clause(neg Head,Body),
        functor(Head,Predicate,_),
        pprint(neg Head,' :-',Body),
        fail.
...
rescind(Predicate/Arity) :-
        ...
        retractall(incompatible(neg Clause,_)),
        retractall(incompatible(_,neg Clause)),
        retractall(sup((Clause := _),_)),
        retractall(sup(_,(Clause := _))),
        retractall(sup((neg Clause := _),_)),
        retractall(sup(_,(neg Clause := _))),
        retractall(sup((Clause :^ _),_)),
        retractall(sup(_,(Clause :^ _))),
        retractall(sup((neg Clause :^ _),_)),
        retractall(sup(_,(neg Clause :^ _))),
