...
class Philosopher extends MyObject implements Runnable {
   ...
   public Philosopher(String name, int id, int napThink, int napEat,
         DiningServer ds) {
      super(name + " " + id);
      this.id = id;
...
class DiningPhilosophers extends MyObject {
   ...
   public static void main(String[] args) {
      ...
      GetOpt go = new GetOpt(args, "USp:R:");
      go.optErr = false;
      String usage = "Usage: -p numPhilosophers -S checkStarving"
      ...
      while ((ch = go.getopt()) != go.optEOF) {
         ...
         else if ((char)ch == 'S') checkStarving = true;
         else if ((char)ch == 'R')
      ...
      System.out.println("DiningPhilosophers: numPhilosophers=" + numPhilosophers
         + ", checkStarving=" + checkStarving + ", runTime=" + runTime);
      ...
      for (int i = 0; i < numPhilosophers; i++) {
         napThink[i] = go.tryArg(argNum++, 8 /* default */);
         napEat[i] = go.tryArg(argNum++, 2 /* default */);
      }
