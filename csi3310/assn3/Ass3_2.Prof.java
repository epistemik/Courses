...
class TunnelController {
	...
	private boolean counting_consect_nb_cars;
	private boolean counting_consect_sb_cars;
	private int consect_nb_cars;
	private int consect_sb_cars;
	
	private int limit_consect_cars;

	public TunnelController(String nb_light, String sb_light, 
	              CarQueue nb_queue, CarQueue sb_queue, int limit_consect_cars)
	{
		...
		this.limit_consect_cars = limit_consect_cars;
		counting_consect_nb_cars = false;
		counting_consect_sb_cars = false;
		consect_nb_cars = 0;
		consect_sb_cars = 0;
		display_lights();
	}
	...
	{
		if (counting_consect_nb_cars && consect_nb_cars >= limit_consect_cars) {
			//do not enter the tunnel
			if (nb_light=="GREEN") {
				nb_light = "RED";
				counting_consect_sb_cars = true;
				consect_sb_cars = 0; 
				display_lights();
			}
			...
			if (sb_car_in_tunnel==0) {//then enter the tunnel
				nb_car_in_tunnel++;
				if (counting_consect_nb_cars) {consect_nb_cars++;}
				if (nb_light=="RED") {
					nb_light = "GREEN";
					counting_consect_sb_cars = false;
					display_lights();
	...
	{
		if (counting_consect_sb_cars && consect_sb_cars >= limit_consect_cars) {
			//do not enter the tunnel
			if (sb_light=="GREEN") {
				sb_light = "RED";
				counting_consect_nb_cars = true;
				consect_nb_cars = 0; 
				display_lights();
			}
		} else {
			if (nb_car_in_tunnel==0) {//then enter the tunnel
				sb_car_in_tunnel++;
				if (counting_consect_sb_cars) {consect_sb_cars++;}
				if (sb_light=="RED") {
					sb_light = "GREEN";
					counting_consect_nb_cars = false;
					display_lights();
...
public class Ass3_2 {
	...
	public static void main(String args[]) 
	{
		...
		final int LIMIT_CONSECT_CARS = 5;

		CarQueue nb_queue = new CarQueue();
		CarQueue sb_queue = new CarQueue();

		TunnelController tc = new TunnelController("GREEN", "GREEN", 
					nb_queue, sb_queue, LIMIT_CONSECT_CARS);
