# CALCULATE THE TOTAL MARKS AND LIST THEM IN ORDER FROM HIGHEST TO LOWEST
...
while (<MARKS>) {
  ...
  # create hashes with student number as the key
  # concatenate the assignment marks into one string as the value of hash %assns
...
while (<FINAL>) {
  ...
  #put grade in a separate hash
  $grades{$studnum} = $grade ;
  }

#use default variable $_ to loop through %assns and %exams
foreach (sort keys %assns) {
  ...
  # create a new hash with studnum as the key and total marks as the value of each entry
  # cannot have total marks as the key because identical totals will be overwritten
  $totals{$_} = $overall ;
  }

$count = 1 ;
$sum = 0;
# print out the rank, total, studnum, and grade from lowest stud num to highest
# have to specify a sort on the values rather than the keys, and do an explicit
# numeric sort, and sort in reverse to get highest to lowest
foreach (reverse sort { $totals{$a} <=> $totals{$b} } keys %totals) {
  printf "%d/%6.2f:\t\t%d\t\t%s\n", $count, $totals{$_}, $_, $grades{$_} ;
  $count++ ;
  $sum += $totals{$_} ;
  }
$ave = $sum/$count ;
