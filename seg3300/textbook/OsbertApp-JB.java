...
class OsbertApplication
{
	...
	//
	// main function for the Osbert application
	//
	// Parameters:	args[] = arguments passed to the application from the command line.
	// These are not required by this particular application, but are required by the Java compiler.
	//
	{
	...
	//
	{
		try
		{
			...
			while (inFile.getFilePointer () != inFile.length ())
			{
				...
				int			compareLastNames = OsbertUtilities.removeQuestionMarks
						(tempLN.toString ()).compareTo (OsbertUtilities.removeQuestionMarks (ln));
